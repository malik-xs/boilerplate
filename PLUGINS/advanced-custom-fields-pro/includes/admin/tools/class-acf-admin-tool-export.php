...
class ACF_Admin_Tool_Export extends ACF_Admin_Tool {
	...
	*/
	...
	function initialize() {
		...
		// vars
		$this->name = 'export';
		$this->title = __("Export Field Groups", 'boilerplate');
    	
    	
    	// active
    	if( $this->is_active() ) {
			$this->title .= ' - ' . __('Generate PHP', 'boilerplate');
		}
	...
	*/
	...
	function submit_download() {
		...
		// validate
		if( $json === false ) {
			return acf_add_admin_notice( __("No field groups selected", 'boilerplate'), 'warning' );
		}
	...
	function load() {
    	...
    	if( $this->is_active() ) {
	    	...
	    	if( $selected ) {
		    	...
		    	$text = sprintf( _n( 'Exported 1 field group.', 'Exported %s field groups.', $count, 'boilerplate' ), $count );
	...
	*/
	...
	function html_field_selection() {
		...
		acf_render_field_wrap(array(
			'label'		=> __('Select Field Groups', 'boilerplate'),
			'type'		=> 'checkbox',
			'name'		=> 'keys',
			'prefix'	=> false,
			'value'		=> $selected,
	...
	*/
	...
	function html_panel_selection() {
		...
		<div class="acf-panel acf-panel-selection">
			<h3 class="acf-panel-title"><?php _e('Select Field Groups', 'boilerplate') ?> <i class="dashicons dashicons-arrow-right"></i></h3>
	...
	*/
	...
	function html_panel_settings() {
		...
		<div class="acf-panel acf-panel-settings">
			<h3 class="acf-panel-title"><?php _e('Settings', 'boilerplate') ?> <i class="dashicons dashicons-arrow-right"></i></h3>
			<div class="acf-panel-inside">
				<?php 
...
*/
	...
	*/
	...
	function html_archive() {
		...
		?>
		<p><?php _e('Select the field groups you would like to export and then select your export method. Use the download button to export to a .json file which you can then import to another ACF installation. Use the generate button to export to PHP code which you can place in your theme.', 'boilerplate'); ?></p>
		<div class="acf-fields">
			<?php $this->html_field_selection(); ?>
		</div>
		<p class="acf-submit">
			<button type="submit" name="action" class="button button-primary" value="download"><?php _e('Export File', 'boilerplate'); ?></button>
			<button type="submit" name="action" class="button" value="generate"><?php _e('Generate PHP', 'boilerplate'); ?></button>
		</p>
		<?php
	...
	function html_single() {
		...
		<div class="acf-postbox-columns">
			...
			<div class="acf-postbox-side">
				...
				<p class="acf-submit">
					<button type="submit" name="action" class="button button-primary" value="generate"><?php _e('Generate PHP', 'boilerplate'); ?></button>
				</p>
			</div>
		</div>
		<?php
	...
	*/
	...
	function html_generate() {
		...
		?></textarea>
		<p class="acf-submit">
			<a class="button" id="acf-export-copy"><?php _e( 'Copy to clipboard', 'boilerplate' ); ?></a>
		</p>
		...
		(function($){
			...
			$a.on('click', function( e ){
				...
				try {
					...
					// tooltip
					acf.newTooltip({
						text: 		"<?php _e('Copied', 'boilerplate' ); ?>",
						timeout:	250,
						target: 	$(this),
					});
