...
class ACF_Admin_Field_Groups {
	...
	public function current_screen() {
		...
		$wp_post_statuses['publish']->label_count = _n_noop( 'Active <span class="count">(%s)</span>', 'Active <span class="count">(%s)</span>', 'boilerplate' );
	...
	public function admin_enqueue_scripts() {
		...
		acf_localize_text(array(
			'Review local JSON changes' => __( 'Review local JSON changes', 'boilerplate' ),
			'Loading diff'              => __( 'Loading diff', 'boilerplate' ),
			'Sync changes'              => __( 'Sync changes', 'boilerplate' ),
		));
	}
	...
	public function get_disabled_post_state() {
		return '<span class="dashicons dashicons-hidden"></span> ' . _x( 'Disabled', 'post status', 'boilerplate' );
	}
	...
	public function admin_table_columns( $_columns ) {
		$columns = array(
			...
			'acf-description'	=> __('Description', 'boilerplate'),
			'acf-key'			=> __('Key', 'boilerplate'),
			'acf-location' 		=> __('Location', 'boilerplate'),
			'acf-count' 		=> __('Fields', 'boilerplate'),
		);
		if( acf_get_local_json_files() ) {
			$columns['acf-json'] = __('Local JSON', 'boilerplate');
		}
		return $columns;
	}
	...
	public function render_admin_table_column_locations( $field_group ) {
		...
		if( $objects ) {
			...
			$html = '<span class="dashicons dashicons-businesswoman"></span> ' . __( 'Various', 'boilerplate' );
		}
	...
	public function get_human_readable_file_location( $file ) {
		...
		if( strpos($file, $theme_path) !== false ) {
			...
			$located = sprintf( __('Located in theme: %s', 'boilerplate'), $rel_file );
			
		} elseif( strpos($file, WP_PLUGIN_DIR) !== false ) {
			$rel_file = str_replace( WP_PLUGIN_DIR, '', $file );
			$located = sprintf( __('Located in plugin: %s', 'boilerplate'), $rel_file );
		
		} else {
			$rel_file = str_replace( ABSPATH, '', $file );
			$located = sprintf( __('Located in: %s', 'boilerplate'), $rel_file );
		}
		return $located;
	}
	...
	public function render_admin_table_column_local_status( $field_group ) {
		...
		if( isset( $json[ $field_group['key'] ] ) ) {
			...
			if( isset($this->sync[ $field_group['key'] ]) ) {
				...
				echo '<strong>' . __( 'Sync available', 'boilerplate' ) . '</strong>';
				if( $field_group['ID'] ) {
					echo '<div class="row-actions">
						<span class="sync"><a href="' . esc_url( $url ) . '">' . __( 'Sync', 'boilerplate' ) . '</a> | </span>
						<span class="review"><a href="#" data-event="review-sync" data-id="' . esc_attr($field_group['ID']) . '" data-href="' . esc_url( $url ) . '">' . __( 'Review changes', 'boilerplate' ) . '</a></span>
					</div>';
				} else {
					echo '<div class="row-actions">
						<span class="sync"><a href="' . esc_url( $url ) . '">' . __( 'Import', 'boilerplate' ) . '</a></span>
					</div>';
				}
			} else {
				echo __( 'Saved', 'boilerplate' );
			}
		} else {
			echo '<span class="acf-secondary-text">' . __( 'Awaiting save', 'boilerplate' ) . '</span>';
		}
	}
	...
	public function page_row_actions( $actions, $post ) {
		...
		$actions[ 'acfduplicate' ] = '<a href="' . esc_url( $duplicate_action_url ) . '" aria-label="' . esc_attr__( 'Duplicate this item', 'boilerplate' ) . '">' . __( 'Duplicate', 'boilerplate' ) . '</a>';
	...
	public function admin_table_bulk_actions( $actions ) {
		...
		if( $this->view !== 'sync' ) {
			$actions[ 'acfduplicate' ] = __( 'Duplicate', 'boilerplate' );
		}
		...
		if( $this->sync ) {
			...
			$actions[ 'acfsync' ] = __( 'Sync changes', 'boilerplate' );
		}
		return $actions;
	}
	...
	public function check_duplicate() {
		...
		if( isset($_GET['acfduplicatecomplete']) ) {
			...
			// Generate text.
			$text = sprintf( 
				_n( 'Field group duplicated.', '%s field groups duplicated.', count($ids), 'boilerplate' ),
				count($ids)
			);
	...
	public function check_sync() {
		...
		if( isset($_GET['acfsynccomplete']) ) {
			...
			// Generate text.
			$text = sprintf( 
				_n( 'Field group synchronised.', '%s field groups synchronised.', count($ids), 'boilerplate' ),
				count($ids)
			);
	...
	public function admin_table_views( $views ) {
		...
		if( $count ) {
			$views['sync'] = sprintf(
				...
				esc_html( __('Sync available', 'boilerplate') ),
				$count
			);
		}
	...
	foreach( $this->sync as $k => $field_group ) {
		...
		foreach( $columns as $column_name => $column_label ) {
			...
			switch ( $column_name ) {
				...
				// Checkbox.
				case 'cb':
					echo '<label for="cb-select-' . esc_attr( $k ) .'" class="screen-reader-text">' .esc_html( sprintf( __( 'Select %s', 'boilerplate' ), $field_group['title'] ) ) . '</label>';
