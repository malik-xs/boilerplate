...
class acf_admin_field_group {
	...
	*/
	...
	function post_updated_messages( $messages ) {
		...
		$messages['acf-field-group'] = array(
			...
			1 => __('Field group updated.', 'boilerplate'),
			2 => __('Field group updated.', 'boilerplate'),
			3 => __('Field group deleted.', 'boilerplate'),
			4 => __('Field group updated.', 'boilerplate'),
			5 => false, // field group does not support revisions
			6 => __('Field group published.', 'boilerplate'),
			7 => __('Field group saved.', 'boilerplate'),
			8 => __('Field group submitted.', 'boilerplate'),
			9 => __('Field group scheduled for.', 'boilerplate'),
			10 => __('Field group draft updated.', 'boilerplate')
		);
	...
	*/
	...
	function admin_enqueue_scripts() {
		...
		acf_localize_text(array(
			'The string "field_" may not be used at the start of a field name'	=> __('The string "field_" may not be used at the start of a field name', 'boilerplate'),
			'This field cannot be moved until its changes have been saved'		=> __('This field cannot be moved until its changes have been saved', 'boilerplate'),
			'Field group title is required'										=> __('Field group title is required', 'boilerplate'),
			'Move to trash. Are you sure?'										=> __('Move to trash. Are you sure?', 'boilerplate'),
			'No toggle fields available'										=> __('No toggle fields available', 'boilerplate'),
			'Move Custom Field'													=> __('Move Custom Field', 'boilerplate'),
			'Checked'															=> __('Checked', 'boilerplate'),
			'(no label)'														=> __('(no label)', 'boilerplate'),
			'(this field)'														=> __('(this field)', 'boilerplate'),
			'copy'																=> __('copy', 'boilerplate'),
			'or'																=> __('or', 'boilerplate'),
			'Null'																=> __('Null', 'boilerplate'),
			
			// Conditions
			'Has any value'				=> __('Has any value', 'boilerplate'),
			'Has no value'				=> __('Has no value', 'boilerplate'),
			'Value is equal to'			=> __('Value is equal to', 'boilerplate'),
			'Value is not equal to'		=> __('Value is not equal to', 'boilerplate'),
			'Value matches pattern'		=> __('Value matches pattern', 'boilerplate'),
			'Value contains'			=> __('Value contains', 'boilerplate'),
			'Value is greater than'		=> __('Value is greater than', 'boilerplate'),
			'Value is less than'		=> __('Value is less than', 'boilerplate'),
			'Selection is greater than'	=> __('Selection is greater than', 'boilerplate'),
			'Selection is less than'	=> __('Selection is less than', 'boilerplate'),
		));
	...
	*/
	...
	function admin_head() {
		...
		// metaboxes
		add_meta_box('acf-field-group-fields', __("Fields",'boilerplate'), array($this, 'mb_fields'), 'acf-field-group', 'normal', 'high');
		add_meta_box('acf-field-group-locations', __("Location",'boilerplate'), array($this, 'mb_locations'), 'acf-field-group', 'normal', 'high');
		add_meta_box('acf-field-group-options', __("Settings",'boilerplate'), array($this, 'mb_options'), 'acf-field-group', 'normal', 'high');
	...
	*/
	...
	function screen_settings( $html ) {
	    ...
	    $html .= '<label for="acf-field-key-hide"><input id="acf-field-key-hide" type="checkbox" value="1" name="show_field_keys" ' . $checked . ' /> ' . __('Field Keys','boilerplate') . '</label>';
		$html .= '</div>';
	...
	*/
	...
	function post_submitbox_misc_actions() {
		...
		// vars
		$status = $field_group['active'] ? __("Active",'boilerplate') : __("Inactive",'boilerplate');
	...
	*/
	...
	function ajax_move_field() {
		...
		if( $args['field_id'] && $args['field_group_id'] ) {
			...
			echo '<p><strong>' . __('Move Complete.', 'boilerplate') . '</strong></p>';
			echo '<p>' . sprintf( __('The %s field can now be found in the %s field group', 'boilerplate'), $field['label'], $a ). '</p>';
			echo '<a href="#" class="button button-primary acf-close-popup">' . __("Close Window",'boilerplate') . '</a>';
			die();
		...
		echo '<p>' . __('Please select the destination for this field', 'boilerplate') . '</p>';
		...
		echo '<form id="acf-move-field-form">';
			...
			echo '<button type="submit" class="button button-primary">' . __("Move Field",'boilerplate') . '</button>';
