...
function acf_validate_save_post( $show_errors = false ) {
	...
	if( $show_errors ) {
		...
		$message = '<h2>' . __('Validation failed', 'boilerplate') . '</h2>';
		$message .= '<ul>';
		...
		// die
		wp_die( $message, __('Validation failed', 'boilerplate') );
...
function acf_validate_value( $value, $field, $input ) {
	...
	// vars
	$valid = true;
	$message = sprintf( __( '%s value is required', 'boilerplate' ), $field['label'] );
