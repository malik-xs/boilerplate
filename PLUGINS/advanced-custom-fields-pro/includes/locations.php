...
function acf_register_location_type( $class_name ) {
	...
	if( !class_exists($class_name) ) {
		$message = sprintf( __( 'Class "%s" does not exist.', 'boilerplate' ), $class_name );
	...
	if( $store->has( $name ) ) {
		$message = sprintf( __( 'Location type "%s" is already registered.', 'boilerplate' ), $name );
		_doing_it_wrong( __FUNCTION__, $message, '5.9.0' );
		return false;
	}
...
function acf_get_location_rule_types() {
	...
	// Default categories.
	$categories = array(
		'post'		=> __('Post', 'boilerplate'),
		'page'		=> __('Page', 'boilerplate'),
		'user'		=> __('User', 'boilerplate'),
		'forms'		=> __('Forms', 'boilerplate'),
	);
