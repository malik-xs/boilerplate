...
function acf_get_image_sizes() {
	...
	// vars
	$sizes = array(
		'thumbnail'	=>	__("Thumbnail",'boilerplate'),
		'medium'	=>	__("Medium",'boilerplate'),
		'large'		=>	__("Large",'boilerplate')
	);
	...
	// add full end
	$sizes['full'] = __("Full Size",'boilerplate');
...
function acf_validate_attachment( $attachment, $field, $context = 'prepare' ) {
	...
	if( $file['width'] || $file['height'] ) {
		...
		if( $file['width'] ) {
			...
			if( $min_width && $file['width'] < $min_width ) {
				...
				// min width
				$errors['min_width'] = sprintf(__('Image width must be at least %dpx.', 'boilerplate'), $min_width );
				
			} elseif( $max_width && $file['width'] > $max_width ) {
				
				// min width
				$errors['max_width'] = sprintf(__('Image width must not exceed %dpx.', 'boilerplate'), $max_width );
		...
		if( $file['height'] ) {
			...
			if( $min_height && $file['height'] < $min_height ) {
				...
				// min height
				$errors['min_height'] = sprintf(__('Image height must be at least %dpx.', 'boilerplate'), $min_height );
				
			}  elseif( $max_height && $file['height'] > $max_height ) {
				
				// min height
				$errors['max_height'] = sprintf(__('Image height must not exceed %dpx.', 'boilerplate'), $max_height );
	...
	if( $file['size'] ) {
		...
		if( $min_size && $file['size'] < acf_get_filesize($min_size) ) {
			...
			// min width
			$errors['min_size'] = sprintf(__('File size must be at least %s.', 'boilerplate'), acf_format_filesize($min_size) );
			
		} elseif( $max_size && $file['size'] > acf_get_filesize($max_size) ) {
				
			// min width
			$errors['max_size'] = sprintf(__('File size must not exceed %s.', 'boilerplate'), acf_format_filesize($max_size) );
	...
	if( $file['type'] ) {
		...
		if( !empty($mime_types) && !in_array($file['type'], $mime_types) ) {
			...
			if( count($mime_types) > 1 ) {
				...
				$mime_types[] = $last2 . ' ' . __('or', 'boilerplate') . ' ' . $last1;
				
			}
			
			$errors['mime_types'] = sprintf(__('File type must be %s.', 'boilerplate'), implode(', ', $mime_types) );
