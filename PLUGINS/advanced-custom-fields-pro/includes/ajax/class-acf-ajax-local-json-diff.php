...
class ACF_Ajax_Local_JSON_Diff extends ACF_Ajax {
	...
	function get_response( $request ) {
		...
		if( !$id ) {
			return new WP_Error( 'acf_invalid_param', __( 'Invalid field group parameter(s).', 'boilerplate' ), array( 'status' => 404 ) );
		}
		...
		if( !$field_group ) {
			return new WP_Error( 'acf_invalid_id', __( 'Invalid field group ID.', 'boilerplate' ), array( 'status' => 404 ) );
		}
		...
		$key = $field_group['key'];
		if( !isset( $files[ $key ] ) ) {
			return new WP_Error( 'acf_cannot_compare', __( 'Sorry, this field group is unavailable for diff comparison.', 'boilerplate' ), array( 'status' => 404 ) );
		}
		$local_field_group = json_decode( file_get_contents( $files[ $key ] ), true );
		
		// Render diff HTML.
		$date_format = get_option( 'date_format' ) . ' ' . get_option( 'time_format' );
		$date_template = __( 'Last updated: %s', 'boilerplate' );
		$json['html'] = '
		<div class="acf-diff">
			<div class="acf-diff-title">
				<div class="acf-diff-title-left">
					<strong>' . __( 'Original field group', 'boilerplate' ) .  '</strong>
					<span>' . sprintf( $date_template, wp_date( $date_format, $field_group['modified'] ) ) . '</span>
				</div>
				<div class="acf-diff-title-right">
					<strong>' . __( 'JSON field group (newer)', 'boilerplate' ) .  '</strong>
