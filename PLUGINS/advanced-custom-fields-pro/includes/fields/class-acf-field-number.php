...
class acf_field_number extends acf_field {
	...
	*/
	...
	function initialize() {
		...
		// vars
		$this->name = 'number';
		$this->label = __("Number",'boilerplate');
		$this->defaults = array(
			'default_value'	=> '',
			'min'			=> '',
			'max'			=> '',
	...
	*/
	...
	function render_field_settings( $field ) {
		...
		// min
		acf_render_field_setting( $field, array(
			'label'			=> __('Minimum Value','boilerplate'),
			'instructions'	=> '',
			'type'			=> 'number',
			'name'			=> 'min',
		));
		...
		// max
		acf_render_field_setting( $field, array(
			'label'			=> __('Maximum Value','boilerplate'),
			'instructions'	=> '',
			'type'			=> 'number',
			'name'			=> 'max',
		));
		...
		// max
		acf_render_field_setting( $field, array(
			'label'			=> __('Step Size','boilerplate'),
			'instructions'	=> '',
			'type'			=> 'number',
			'name'			=> 'step',
		));
	...
	function validate_value( $valid, $value, $field, $input ){
		...
		if( !is_numeric($value) ) {
			...
			if( !empty($value) ) {
				...
				$valid = __('Value must be a number', 'boilerplate');
		...
		if( is_numeric($field['min']) && $value < floatval($field['min'])) {
			...
			$valid = sprintf(__('Value must be equal to or higher than %d', 'boilerplate'), $field['min'] );
		...
		if( is_numeric($field['max']) && $value > floatval($field['max']) ) {
			...
			$valid = sprintf(__('Value must be equal to or lower than %d', 'boilerplate'), $field['max'] );
