...
class acf_field_relationship extends acf_field {
	...
	*/
	...
	function initialize() {
		...
		$this->label = __("Relationship",'boilerplate');
	...
	*/
	...
	function input_admin_enqueue_scripts() {
		...
		acf_localize_text(array(
			...
			'Maximum values reached ( {max} values )'	=> __('Maximum values reached ( {max} values )', 'boilerplate'),
			'Loading'									=> __('Loading', 'boilerplate'),
			'No matches found'							=> __('No matches found', 'boilerplate'),
	   	));
	}
	...
	*/
	...
	function render_field( $field ) {
		...
		if( in_array('post_type', $filters) ) {
			...
			$filter_post_type_choices = array(
				''	=> __('Select post type', 'boilerplate')
		...
		if( in_array('taxonomy', $filters) ) {
			...
			$term_choices = array();
			$filter_taxonomy_choices = array(
				''	=> __('Select taxonomy', 'boilerplate')
			);
...
<div <?php acf_esc_attr_e($atts); ?>>
	...
	<div class="filters -f<?php echo esc_attr($filter_count); ?>">
		...
		<div class="filter -search">
			<?php acf_text_input( array('placeholder' => __("Search...",'boilerplate'), 'data-filter' => 's') ); ?>
		</div>
		<?php endif; 
	...
	*/
	...
	function render_field_settings( $field ) {
		...
		acf_render_field_setting( $field, array(
			'label'			=> __('Filter by Post Type','boilerplate'),
			'instructions'	=> '',
			'type'			=> 'select',
			'name'			=> 'post_type',
			...
			'ui'			=> 1,
			'allow_null'	=> 1,
			'placeholder'	=> __("All post types",'boilerplate'),
		));
		
		
		// taxonomy
		acf_render_field_setting( $field, array(
			'label'			=> __('Filter by Taxonomy','boilerplate'),
			'instructions'	=> '',
			'type'			=> 'select',
			'name'			=> 'taxonomy',
			...
			'ui'			=> 1,
			'allow_null'	=> 1,
			'placeholder'	=> __("All taxonomies",'boilerplate'),
		));
		
		
		// filters
		acf_render_field_setting( $field, array(
			'label'			=> __('Filters','boilerplate'),
			'instructions'	=> '',
			'type'			=> 'checkbox',
			'name'			=> 'filters',
			'choices'		=> array(
				'search'		=> __("Search",'boilerplate'),
				'post_type'		=> __("Post Type",'boilerplate'),
				'taxonomy'		=> __("Taxonomy",'boilerplate'),
			),
		));
		...
		// filters
		acf_render_field_setting( $field, array(
			'label'			=> __('Elements','boilerplate'),
			'instructions'	=> __('Selected elements will be displayed in each result','boilerplate'),
			'type'			=> 'checkbox',
			'name'			=> 'elements',
			'choices'		=> array(
		...
		// min
		acf_render_field_setting( $field, array(
			'label'			=> __('Minimum posts','boilerplate'),
			'instructions'	=> '',
			'type'			=> 'number',
			'name'			=> 'min',
		));
		...
		// max
		acf_render_field_setting( $field, array(
			'label'			=> __('Maximum posts','boilerplate'),
			'instructions'	=> '',
			'type'			=> 'number',
			'name'			=> 'max',
		));
		...
		acf_render_field_setting( $field, array(
			...
			'name'			=> 'return_format',
			'choices'		=> array(
				'object'		=> __("Post Object",'boilerplate'),
				'id'			=> __("Post ID",'boilerplate'),
			),
			'layout'	=>	'horizontal',
		));
