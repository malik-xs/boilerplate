...
class ACF_Field_User extends ACF_Field {
	...
	function initialize() {
		...
		// Props.
		$this->name = 'user';
		$this->label = __("User",'boilerplate');
	...
	function render_field_settings( $field ) {
		...
		acf_render_field_setting( $field, array(
			'label'			=> __('Filter by role','boilerplate'),
			'instructions'	=> '',
			'type'			=> 'select',
			'name'			=> 'role',
			...
			'ui'			=> 1,
			'allow_null'	=> 1,
			'placeholder'	=> __("All user roles",'boilerplate'),
		));
		...
		acf_render_field_setting( $field, array(
			...
			'name'			=> 'return_format',
			'choices'		=> array(
				'array'			=> __("User Array",'boilerplate'),
				'object'		=> __("User Object",'boilerplate'),
				'id'			=> __("User ID",'boilerplate'),
			),
			'layout'	=>	'horizontal',
		));
	}
	...
	function ajax_query_init( $request, $query ) {
		...
		// Require field.
		if( !$query->field ) {
			$query->send( new WP_Error( 'acf_missing_field', __( 'Error loading field.', 'boilerplate' ), array( 'status' => 404 ) ) );
		}
	}
