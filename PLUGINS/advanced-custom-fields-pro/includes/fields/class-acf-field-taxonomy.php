...
class acf_field_taxonomy extends acf_field {
	...
	*/
	...
	function initialize() {
		...
		// vars
		$this->name = 'taxonomy';
		$this->label = __("Taxonomy",'boilerplate');
...
<div <?php acf_esc_attr_e($div); ?>>
	...
	*/
	...
	function render_field_checkbox( $field ) {
		...
		$args = array(
			'taxonomy'     		=> $field['taxonomy'],
			'show_option_none'	=> sprintf( _x('No %s', 'No terms', 'boilerplate'), strtolower($taxonomy_obj->labels->name) ),
			'hide_empty'   		=> false,
			'style'        		=> 'none',
	...
	*/
	...
	function render_field_settings( $field ) {
		...
		acf_render_field_setting( $field, array(
			'label'			=> __('Taxonomy','boilerplate'),
			'instructions'	=> __('Select the taxonomy to be displayed','boilerplate'),
			'type'			=> 'select',
			'name'			=> 'taxonomy',
		...
		// field_type
		acf_render_field_setting( $field, array(
			'label'			=> __('Appearance','boilerplate'),
			'instructions'	=> __('Select the appearance of this field','boilerplate'),
			'type'			=> 'select',
			'name'			=> 'field_type',
			'optgroup'		=> true,
			'choices'		=> array(
				__("Multiple Values",'boilerplate') => array(
					'checkbox' => __('Checkbox', 'boilerplate'),
					'multi_select' => __('Multi Select', 'boilerplate')
				),
				__("Single Value",'boilerplate') => array(
					'radio' => __('Radio Buttons', 'boilerplate'),
					'select' => _x('Select', 'noun', 'boilerplate')
				)
			)
		));
		...
		// add_term
		acf_render_field_setting( $field, array(
			'label'			=> __('Create Terms','boilerplate'),
			'instructions'	=> __('Allow new terms to be created whilst editing','boilerplate'),
			'name'			=> 'add_term',
			'type'			=> 'true_false',
			'ui'			=> 1,
		));
		...
		// save_terms
		acf_render_field_setting( $field, array(
			'label'			=> __('Save Terms','boilerplate'),
			'instructions'	=> __('Connect selected terms to the post','boilerplate'),
			'name'			=> 'save_terms',
			'type'			=> 'true_false',
			'ui'			=> 1,
		));
		...
		// load_terms
		acf_render_field_setting( $field, array(
			'label'			=> __('Load Terms','boilerplate'),
			'instructions'	=> __('Load value from posts terms','boilerplate'),
			'name'			=> 'load_terms',
			'type'			=> 'true_false',
			'ui'			=> 1,
		));
		...
		// return_format
		acf_render_field_setting( $field, array(
			'label'			=> __('Return Value','boilerplate'),
			'instructions'	=> '',
			'type'			=> 'radio',
			'name'			=> 'return_format',
			'choices'		=> array(
				'object'		=>	__("Term Object",'boilerplate'),
				'id'			=>	__("Term ID",'boilerplate')
			),
			'layout'	=>	'horizontal',
		));
	...
	function ajax_add_term() {
		...
		if( !current_user_can( $taxonomy_obj->cap->manage_terms) ) {
			wp_send_json_error(array(
				'error'	=> sprintf( __('User unable to add new %s', 'boilerplate'), $taxonomy_label )
			));
		}
		...
		if( $args['term_name'] ) {
			...
			if( term_exists($args['term_name'], $field['taxonomy'], $args['term_parent']) ) {
				wp_send_json_error(array(
					'error'	=> sprintf( __('%s already exists', 'boilerplate'), $taxonomy_label )
				));
			}
			...
			wp_send_json_success(array(
				'message'		=> sprintf( __('%s added', 'boilerplate'), $taxonomy_label ),
		...
		?><p class="acf-submit">
			<button class="acf-submit-button button button-primary" type="submit"><?php _e("Add", 'boilerplate'); ?></button>
		</p>
		</form><?php
