...
class ACF_Admin_Updates {
	...
	function display_wp_error( $wp_error ) {
		...
		acf_new_admin_notice(array(
			'text'	=> __('<b>Error</b>. Could not connect to update server', 'boilerplate') . ' <span class="description">(' . esc_html( $wp_error->get_error_message() ) . ').</span>',
			'type'	=> 'error'
		));	
	}
	...
	function admin_menu() {
		...
		// Add submenu.
		$page = add_submenu_page( 'edit.php?post_type=acf-field-group', __('Updates','boilerplate'), __('Updates','boilerplate'), acf_get_setting('capability'), 'acf-settings-updates', array($this,'html') );
		...
		if( version_compare($info['version'], $version, '>') ) {
        	...
        	if( $license ) {
	        	...
	        	if( $update && !$update['package'] ) {
		        	...
		        	acf_new_admin_notice(array(
						'text'	=> __('<b>Error</b>. Could not authenticate update package. Please check again or deactivate and reactivate your ACF PRO license.', 'boilerplate'),
						'type'	=> 'error'
					));	
	        	}
