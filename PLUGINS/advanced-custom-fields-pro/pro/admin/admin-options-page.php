...
class acf_admin_options_page {
	...
	*/
	...
	function admin_head() {
		...
		// add submit div
		add_meta_box('submitdiv', __('Publish','boilerplate'), array($this, 'postbox_submitdiv'), 'acf_options_page', 'side', 'high');
		
		
		
		if( empty($field_groups) ) {
			
			acf_add_admin_notice( sprintf( __('No Custom Field Groups found for this options page. <a href="%s">Create a Custom Field Group</a>', 'boilerplate'), admin_url('post-new.php?post_type=acf-field-group') ), 'warning' );
	...
	*/
	...
	function postbox_acf( $post, $args ) {
		...
		$o = array(
			...
			'editLink'		=> '',
			'editTitle'		=> __('Edit field group', 'boilerplate'),
			'visibility'	=> true
		);
