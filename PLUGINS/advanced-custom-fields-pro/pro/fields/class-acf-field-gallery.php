...
class acf_field_gallery extends acf_field {
	...
	*/
	...
	function initialize() {
		...
		// vars
		$this->name = 'gallery';
		$this->label = __("Gallery",'boilerplate');
	...
	*/
	...
	function input_admin_enqueue_scripts() {
		...
		acf_localize_text(array(
		   	'Add Image to Gallery'		=> __('Add Image to Gallery', 'boilerplate'),
			'Maximum selection reached'	=> __('Maximum selection reached', 'boilerplate'),
	   	));
	}
	...
	*/
	...
	function render_attachment( $id, $field ) {
		...
		if( $attachment['type'] === 'audio' ) {
			$dimensions = __('Length', 'boilerplate') . ': ' . $attachment['fileLength'];	
		...
		<div class="acf-gallery-side-info">
			...
			<p class="actions">
				<a href="#" class="acf-gallery-edit" data-id="<?php echo esc_attr($id); ?>"><?php _e('Edit', 'boilerplate'); ?></a>
				<a href="#" class="acf-gallery-remove" data-id="<?php echo esc_attr($id); ?>"><?php _e('Remove', 'boilerplate'); ?></a>
			</p>
		</div>
...
<div <?php acf_esc_attr_e($attrs); ?>>
	...
	<div class="acf-gallery-main">
			...
			<?php if( $attachments ): ?>
				<?php foreach( $attachments as $i => $attachment ): 
					...
					<div class="<?php echo esc_attr($a_class); ?>" data-id="<?php echo esc_attr($a_id); ?>">
						...
						<div class="actions">
							<a class="acf-icon -cancel dark acf-gallery-remove" href="#" data-id="<?php echo esc_attr($a_id); ?>" title="<?php _e('Remove', 'boilerplate'); ?>"></a>
						</div>
					</div>
				<?php endforeach; ?>
			<?php endif; ?>
		...
		<div class="acf-gallery-toolbar">
			<ul class="acf-hl">
				<li>
					<a href="#" class="acf-button button button-primary acf-gallery-add"><?php _e('Add to gallery', 'boilerplate'); ?></a>
				</li>
				<li class="acf-fr">
					<select class="acf-gallery-sort">
						<option value=""><?php _e('Bulk actions', 'boilerplate'); ?></option>
						<option value="date"><?php _e('Sort by date uploaded', 'boilerplate'); ?></option>
						<option value="modified"><?php _e('Sort by date modified', 'boilerplate'); ?></option>
						<option value="title"><?php _e('Sort by title', 'boilerplate'); ?></option>
						<option value="reverse"><?php _e('Reverse current order', 'boilerplate'); ?></option>
					</select>
				</li>
			</ul>
		</div>
	...
	<div class="acf-gallery-side">
		<div class="acf-gallery-side-inner">
			...
			<div class="acf-gallery-toolbar">
				<ul class="acf-hl">
					<li>
						<a href="#" class="acf-button button acf-gallery-close"><?php _e('Close', 'boilerplate'); ?></a>
					</li>
					<li class="acf-fr">
						<a class="acf-button button button-primary acf-gallery-update" href="#"><?php _e('Update', 'boilerplate'); ?></a>
					</li>
				</ul>
			</div>
		</div>	
	...
	*/
	...
	function render_field_settings( $field ) {
		...
		acf_render_field_setting( $field, array(
			'label'			=> __('Return Format','boilerplate'),
			'instructions'	=> '',
			'type'			=> 'radio',
			'name'			=> 'return_format',
			'layout'		=> 'horizontal',
			'choices'		=> array(
				'array'			=> __("Image Array",'boilerplate'),
				'url'			=> __("Image URL",'boilerplate'),
				'id'			=> __("Image ID",'boilerplate')
			)
		));
		
		// preview_size
		acf_render_field_setting( $field, array(
			'label'			=> __('Preview Size','boilerplate'),
			'instructions'	=> '',
			'type'			=> 'select',
			'name'			=> 'preview_size',
		...
		// insert
		acf_render_field_setting( $field, array(
			'label'			=> __('Insert','boilerplate'),
			'instructions'	=> __('Specify where new attachments are added','boilerplate'),
			'type'			=> 'select',
			'name'			=> 'insert',
			'choices' 		=> array(
				'append'		=> __('Append to the end', 'boilerplate'),
				'prepend'		=> __('Prepend to the beginning', 'boilerplate')
			)
		));
		
		// library
		acf_render_field_setting( $field, array(
			'label'			=> __('Library','boilerplate'),
			'instructions'	=> __('Limit the media library choice','boilerplate'),
			'type'			=> 'radio',
			'name'			=> 'library',
			'layout'		=> 'horizontal',
			'choices' 		=> array(
				'all'			=> __('All', 'boilerplate'),
				'uploadedTo'	=> __('Uploaded to post', 'boilerplate')
			)
		));
		
		// min
		acf_render_field_setting( $field, array(
			'label'			=> __('Minimum Selection','boilerplate'),
			'instructions'	=> '',
			'type'			=> 'number',
			'name'			=> 'min'
		));
		...
		// max
		acf_render_field_setting( $field, array(
			'label'			=> __('Maximum Selection','boilerplate'),
			'instructions'	=> '',
			'type'			=> 'number',
			'name'			=> 'max'
		));
		...
		// min
		acf_render_field_setting( $field, array(
			'label'			=> __('Minimum','boilerplate'),
			'instructions'	=> __('Restrict which images can be uploaded','boilerplate'),
			'type'			=> 'text',
			'name'			=> 'min_width',
		...
		acf_render_field_setting( $field, array(
			...
			'type'			=> 'text',
			'name'			=> 'min_size',
			'prepend'		=> __('File size', 'boilerplate'),
			'append'		=> 'MB',
			'_append' 		=> 'min_width'
		));	
		...
		// allowed type
		acf_render_field_setting( $field, array(
			'label'			=> __('Allowed file types','boilerplate'),
			'instructions'	=> __('Comma separated list. Leave blank for all types','boilerplate'),
			'type'			=> 'text',
			'name'			=> 'mime_types',
		));
	}
	...
	*/
	...
	function validate_value( $valid, $value, $field, $input ){
		...
		if( count($value) < $field['min'] ) {
			...
			$valid = _n( '%s requires at least %s selection', '%s requires at least %s selections', $field['min'], 'boilerplate' );
