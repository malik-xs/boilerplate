...
class acf_field_repeater extends acf_field {
	...
	*/
	...
	function initialize() {
		...
		// vars
		$this->name = 'repeater';
		$this->label = __("Repeater",'boilerplate');
	...
	*/
	...
	function input_admin_enqueue_scripts() {
		...
		acf_localize_text(array(
		   	'Minimum rows reached ({min} rows)'	=> __('Minimum rows reached ({min} rows)', 'boilerplate'),
			'Maximum rows reached ({max} rows)'	=> __('Maximum rows reached ({max} rows)', 'boilerplate'),
	   	));
	}
	...
	*/
	...
	function render_field( $field ) {
		...
		// button label
		if( $field['button_label'] === '' ) $field['button_label'] = __('Add Row', 'boilerplate');
	...
	<tbody>
		<?php foreach( $value as $i => $row ): 
			...
			<tr class="acf-row<?php if( $i === 'acfcloneindex' ){ echo ' acf-clone'; } ?>" data-id="<?php echo $id; ?>">
				...
				<?php if( $show_order ): ?>
					<td class="acf-row-handle order" title="<?php _e('Drag to reorder','boilerplate'); ?>">
						<?php if( $field['collapsed'] ): ?>
						<a class="acf-icon -collapse small" href="#" data-event="collapse-row" title="<?php _e('Click to toggle','boilerplate'); ?>"></a>
						<?php endif; ?>
				...
				<?php if( $show_remove ): ?>
					<td class="acf-row-handle remove">
						<a class="acf-icon -plus small acf-js-tooltip hide-on-shift" href="#" data-event="add-row" title="<?php _e('Add row','boilerplate'); ?>"></a>
						<a class="acf-icon -duplicate small acf-js-tooltip show-on-shift" href="#" data-event="duplicate-row" title="<?php _e('Duplicate row','boilerplate'); ?>"></a>
						<a class="acf-icon -minus small acf-js-tooltip" href="#" data-event="remove-row" title="<?php _e('Remove row','boilerplate'); ?>"></a>
					</td>
				<?php endif; ?>
	...
	*/
	...
	function render_field_settings( $field ) {
		...
		?><tr class="acf-field acf-field-setting-sub_fields" data-setting="repeater" data-name="sub_fields">
			<td class="acf-label">
				<label><?php _e("Sub Fields",'boilerplate'); ?></label>
		...
		acf_render_field_setting( $field, array(
			'label'			=> __('Collapsed','boilerplate'),
			'instructions'	=> __('Select a sub field to show when row is collapsed','boilerplate'),
			'type'			=> 'select',
			'name'			=> 'collapsed',
			'allow_null'	=> 1,
			'choices'		=> $choices
		...
		// min
		acf_render_field_setting( $field, array(
			'label'			=> __('Minimum Rows','boilerplate'),
			'instructions'	=> '',
			'type'			=> 'number',
			'name'			=> 'min',
			'placeholder'	=> '0',
		...
		// max
		acf_render_field_setting( $field, array(
			'label'			=> __('Maximum Rows','boilerplate'),
			'instructions'	=> '',
			'type'			=> 'number',
			'name'			=> 'max',
			'placeholder'	=> '0',
		...
		acf_render_field_setting( $field, array(
			...
			'type'			=> 'text',
			'name'			=> 'button_label',
			'placeholder'	=> __('Add Row','boilerplate')
		));
	...
	*/
	...
	function validate_value( $valid, $value, $field, $input ){
		...
		if( $min && $count < $min ) {
			...
			// create error
			$error = __('Minimum rows reached ({min} rows)', 'boilerplate');
