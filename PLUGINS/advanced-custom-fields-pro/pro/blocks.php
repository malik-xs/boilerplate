...
function acf_register_block_type( $block ) {
    ...
    // Require name.
    if( !$block['name'] ) {
	    $message = __( 'Block type name is required.', 'boilerplate' );
	...
	if( acf_has_block_type($block['name']) ) {
		$message = sprintf( __( 'Block type "%s" is already registered.', 'boilerplate' ), $block['name'] );
		_doing_it_wrong( __FUNCTION__, $message, '5.8.0' );
		return false;
	}
...
function acf_enqueue_block_assets() {
	...
	acf_localize_text(array(
		'Switch to Edit'		=> __('Switch to Edit', 'boilerplate'),
		'Switch to Preview'		=> __('Switch to Preview', 'boilerplate'),
		'Change content alignment'	=> __('Change content alignment', 'boilerplate'),

		/* translators: %s: Block type title */
		'%s settings'			=> __('%s settings', 'boilerplate'),
	));
